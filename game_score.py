...
def game(player,p1,p2):
    ...
    while winCondition != True or gameEnd == 64:
        ...
        while playerTurn == False:
            layer = eval(
                input("Layer select : Front Layer (0), Front Middle Layer (1), Back Middle Layer (2), Back Layer (3)"))
            ...
            if gameCube[layer, row, cube] == 0:
                ...
                gameEnd += 1
                print("============================================")
                print("Successful Placement: Current Gameboard")
                print(gameCube)
...
def gameWin(gameCube):
    gameCube1D = np.array([gameCube[0, 0, 0], gameCube[0, 0, 1], gameCube[0, 0, 2], gameCube[0, 0, 3],
                           ...
                           gameCube[3, 3, 3]])
    for i in range(0, 4):
        win = gameCube1D[0 + (i * 4)] + gameCube1D[1 + (i * 4)] + gameCube1D[2 + (i * 4)] + gameCube1D[
            3 + (i * 4)]  # horizontal layer wins
        if win == 4 or win == -4:
            return True
        win = gameCube1D[0 + (i * 4)] + gameCube1D[17 + (i * 4)] + gameCube1D[34 + (i * 4)] + gameCube1D[
            51 + (i * 4)]  # -slope horizontal depth wins
        if win == 4 or win == -4:
            return True
        win = gameCube1D[48 + (i * 4)] + gameCube1D[33 + (i * 4)] + gameCube1D[18 + (i * 4)] + gameCube1D[
            3 + (i * 4)]  # +slope horizontal depth wins
        if win == 4 or win == -4:
            return True
        win = gameCube1D[i] + gameCube1D[i + 20] + gameCube1D[i + 40] + gameCube1D[i + 60]  # +slope vertical depth wins
        if win == 4 or win == -4:
            return True
        win = gameCube1D[i + 12] + gameCube1D[i + 24] + gameCube1D[i + 36] + gameCube1D[
            i + 48]  # -slope vertical depth wins
        if win == 4 or win == -4:
            return True
        win=gameCube1D[3 + i * 16] + gameCube1D[6 + i * 16] + gameCube1D[9 + i * 16] + gameCube1D[
            12 + i * 16]  # +slope layer wins
        if win == 4 or win == -4:
            return True
        win = gameCube1D[i * 16] + gameCube1D[5 + i * 16] + gameCube1D[10 + i * 16] + gameCube1D[15 + i * 16]  # -slope layer wins
        if win == 4 or win == -4:
            return True
    for i in range(0, 4):  #
        for j in range(0, 4):
            win = gameCube1D[0 + j + (i * 16)] + gameCube1D[4 + j + (i * 16)] + gameCube1D[8 + j + (i * 16)] + \
                  gameCube1D[12 + j + (i * 16)]  # vertical layer wins
            if win == 4 or win == -4:
                return True
            win = gameCube1D[0 + j + (i * 4)] + gameCube1D[16 + j + (i * 4)] + gameCube1D[32 + j + (i * 4)] + \
                  gameCube1D[48 + j + (i * 4)]  # depth wins
            if win == 4 or win == -4:
                return True
